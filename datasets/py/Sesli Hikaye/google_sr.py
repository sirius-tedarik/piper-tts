...
def recognize_speech_from_files_in_audios():
    # 'audios' klasöründeki tüm wav dosyalarını al
    audio_folder = 'audios'
    ...
    for wav_file in wav_files:
        ...
        try:
            ...
            for result in response.results:
                print(f"{wav_file} - Tanınan metin: {result.alternatives[0].transcript}")
                write_to_metadata(wav_file, result.alternatives[0].transcript)
        except Exception as e:
            print(f"{wav_file} - Google Cloud Speech API'ye bağlanırken hata oluştu: {e}")
